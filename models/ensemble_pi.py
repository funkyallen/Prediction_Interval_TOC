...
from utils.tools import all_to_pi, Piei, cal_pi_index
import time


class Bootstrap_PI():
    def __init__(self, n_ensemble, alphas, weight=[0.5, 1, 0.5], bootstrap_method='prop_of_data', prop_select=0.80):
    ...
    def fit_predict(self, train_x, train_y, test_X, val_data=None):
        ...
        for i in tqdm(range(self.n_ensemble)):
            ...
            if val_data:
                ...
                val_y = self.train_y[perm[:int(
                    perm.shape[0] * 0.2)]]           
            history = tf_pi.fit(fit_x, fit_y, (val_x, val_y), epochs=1000)
...
def multi_alphas_PI(train_x, train_y, test_x, sample_position, pre_position, alphas, weight=[1, 1, 1], bootstrap_method='prop_of_data'):
    model = Bootstrap_PI(len(alphas), alphas, weight=weight,
...
if __name__ == '__main__':
    ...
    from utils.plot_func import plot_multi_boundary
    from utils.plot_func import plot_simple_boundary, plot_pi_toc
